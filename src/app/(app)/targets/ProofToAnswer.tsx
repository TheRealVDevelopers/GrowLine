...
const statusStyle: Record<string, string> = {
  ...
  submitted: "bg-navy-100 text-navy",
  approved: "bg-success-100 text-success",
  rejected: "bg-gold-100 text-gold-600",
};
...
export default function ProofToAnswer({ proof }: { proof: ProofRow }) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl bg-surface p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${
            statusStyle[proof.status] ?? "bg-navy-100 text-navy"
          }`}
        >
          {label}
        </span>
      ...
      {proof.submitNote && (
        <p className="text-sm text-navy/70">You wrote: {proof.submitNote}</p>
      )}
